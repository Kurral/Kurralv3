...
Usage: from ABTester.agent_replay import replay_agent_artifact
...
if __name__ == "__main__":
    ...
    # If we're in ABTester, add parent to path
    if "ABTester" in current_file.parts:
...
from ABTester.artifact_generator import ArtifactGenerator
from ABTester.tool_stubber import ToolStubber, create_stubbed_tool
from ABTester.ars_scorer import calculate_ars
from ABTester.side_effect_config import SideEffectConfig
...
def replay_agent_artifact(
    ...
    try:
        ...
        if agent_folder.name.startswith("level") and "agent" in agent_folder.name:
            try:
                ...
                try:
                    from ABTester.langchain_integration import extract_llm_config_from_langchain
                ...
                from ABTester.langchain_integration import compute_graph_version
                from ABTester.models.kurral import ResolvedPrompt
    ...
    try:
        ...
        from ABTester.config import get_storage_config
        from ABTester.storage import create_storage_backend
...
if __name__ == "__main__":
    ...
    # If ABTester is not in path, try to find project root
    try:
        import ABTester
    except ImportError:
        # Try to find project root by looking for ABTester directory
        current_dir = Path.cwd()
        # Check current dir and parent dirs
        for check_dir in [current_dir, current_dir.parent, current_dir.parent.parent]:
            ABTester_path = check_dir / "ABTester"
            if ABTester_path.exists() and ABTester_path.is_dir():
    ...
    if args.latest:
        ...
        print("Usage: python -m ABTester.agent_replay <kurral_id> [--artifacts-dir <dir>]")
        print("   or: python -m ABTester.agent_replay --latest [--artifacts-dir <dir>]")
        print("   or: python -m ABTester.agent_replay --run-id <run_id> [--artifacts-dir <dir>]")
        print("\nExample: python -m ABTester.agent_replay 810c48c1-fc9c-4ec4-a4f5-7fb7ed86506d")
        print("   or: python -m ABTester.agent_replay 810c48c1 --artifacts-dir level1agentK/artifacts")
