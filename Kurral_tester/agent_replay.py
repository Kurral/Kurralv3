...
Usage: from Kurral_tester.agent_replay import replay_agent_artifact
...
if __name__ == "__main__":
    ...
    # If we're in Kurral_tester, add parent to path
    if "Kurral_tester" in current_file.parts:
    ...
    try:
        ...
        if agent_folder.name.startswith("level") and "agent" in agent_folder.name:
            try:
                ...
                try:
                    from Kurral_tester.langchain_integration import extract_llm_config_from_langchain
                ...
                from Kurral_tester.langchain_integration import compute_graph_version
                from Kurral_tester.models.kurral import ResolvedPrompt
...
if __name__ == "__main__":
    ...
    # If Kurral_tester is not in path, try to find project root
    try:
        import Kurral_tester
    except ImportError:
        # Try to find project root by looking for Kurral_tester directory
        current_dir = Path.cwd()
        # Check current dir and parent dirs
        for check_dir in [current_dir, current_dir.parent, current_dir.parent.parent]:
            kurral_tester_path = check_dir / "Kurral_tester"
            if kurral_tester_path.exists() and kurral_tester_path.is_dir():
    ...
    if args.latest:
        ...
        print("Usage: python -m Kurral_tester.agent_replay <kurral_id> [--artifacts-dir <dir>]")
        print("   or: python -m Kurral_tester.agent_replay --latest [--artifacts-dir <dir>]")
        print("   or: python -m Kurral_tester.agent_replay --run-id <run_id> [--artifacts-dir <dir>]")
        print("\nExample: python -m Kurral_tester.agent_replay 810c48c1-fc9c-4ec4-a4f5-7fb7ed86506d")
        print("   or: python -m Kurral_tester.agent_replay 810c48c1 --artifacts-dir level1agentK/artifacts")
