...
```bash
# Replay by file path
python -m kurral.cli.replay_cmd artifact.kurral

# Replay by run_id
python -m kurral.cli.replay_cmd --run-id my_run_123

# Replay latest artifact
python -m kurral.cli.replay_cmd --latest

# Replay with change detection (B replay if model changed)
python -m kurral.cli.replay_cmd artifact.kurral --current-model gpt-4-turbo

# Show diff between original and replay
python -m kurral.cli.replay_cmd artifact.kurral --diff

# Debug mode
python -m kurral.cli.replay_cmd artifact.kurral --debug
```
